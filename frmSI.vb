Public Class frmSI
    ...
    Private Sub LoadChartOfAccount()
        ...
        query = " SELECT AccntCode, AccountTitle FROM tblDefaultAccount " & _
                ...
                " WHERE Type = 'AR'  AND tblDefaultAccount.Status = 'Active'" & _
    ...
    Private Sub LoadSI(ByVal TransID As String)
        ...
        query = " SELECT   TransID, SI_No, VCECode, DateSI, Remarks,  Terms, " & _
                ...
                "          ISNULL(VATable,1) AS VATable, ISNULL(VATInc,1) AS VATInc, Currency, ISNULL(Exchange_Rate,0) AS Exchange_Rate, " & _
    ...
    Private Sub LoadAccountingEntry(ByVal TransID As Integer)
        Try
            ...
            query = " SELECT JE_No, VCECode, VCEName, AccntCode, AccntTitle, Particulars, RefNo, " & _
                     ...
                     " WHERE  RefType ='SJ' AND RefTransID IN (SELECT TransID FROM tblSJ WHERE Reftype = 'SI' AND  RefID = '" & TransID & "') "
            SQL.ReadQuery(query)
            dgvEntry.Rows.Clear()
    ...
    Protected Sub LoadSIDetails(ByVal TransID As String)
        ...
        query = " SELECT    ItemCode, Description, UOM, QTY, ISNULL(UnitPrice,0) AS UnitPrice, " & _
                ...
                "           ISNULL(VATAmount,0) AS VATAmount, ISNULL(NetAmount,0) AS NetAmount, ISNULL(VATable,1) AS VATable, ISNULL(VATInc,1) AS VATInc, tblSI_Details.Size, tblSI_Details.Color " & _
        ...
        While SQL.SQLDR.Read
            ...
            dgvItemList.Rows.Add(ItemCode, SQL.SQLDR("Description").ToString, SQL.SQLDR("UOM").ToString, _
                                 SQL.SQLDR("QTY").ToString, CDec(SQL.SQLDR("UnitPrice")).ToString("N4"), _
                                 ...
                                 SQL.SQLDR("VATInc").ToString, _
                                 SQL.SQLDR("Size").ToString, _
                                 SQL.SQLDR("Color").ToString)
            LoadUOM(ItemCode, dgvItemList.Rows.Count - 1)
            LoadColor(ItemCode, dgvItemList.Rows.Count - 1)
            LoadSize(ItemCode, dgvItemList.Rows.Count - 1)
        End While
    End Sub
    ...
    Private Sub LoadDR(ByVal DR_No As String)
        Dim query, itemcode As String
        query = " SELECT  TransID, DateDR AS Date, tblDR.VCECode, VCEName , DR_No " & _
                " FROM    tblDR INNER JOIN viewVCE_Master " & _
                " ON      tblDR.VCECode = viewVCE_Master.VCECode " & _
                " WHERE   tblDR.Status ='Active' AND TransID ='" & DR_No & "' "
        SQL.ReadQuery(query)
        If SQL.SQLDR.Read Then
            DR_ID = SQL.SQLDR("TransID")
            txtDRRef.Text = SQL.SQLDR("DR_No")
            txtVCECode.Text = SQL.SQLDR("VCECode").ToString
            ...
            '        " tblDR_Details.QTY, tblDR_Details.UnitPrice, tblDR_Details.GrossAmount,  " & _
            '        " ISNULL(tblDR_Details.VATAmount,0) AS VATAmount, tblDR_Details.NetAmount, tblDR_Details.VATable, tblDR_Details.VATInc" & _
            '        "  FROM tblDR INNER JOIN tblDR_Details " & _
            '        "  ON tblDR.TransID = tblDR_Details.TransID " & _
            '        "  WHERE tblDR.TransID = '" & DR_No & "' "

            query = " SELECT tblDR_Details.ItemCode, tblDR_Details.Description, tblDR_Details.UOM, tblDR_Details.QTY,  " & _
                    " CASE WHEN viewItem_Price.VATable = 0 THEN viewItem_Price.UnitPrice " & _
                    " WHEN VATInclusive = 1 THEN viewItem_Price.UnitPrice/1.12 " & _
                    " ELSE viewItem_Price.UnitPrice END AS UnitPrice, " & _
                    " tblDR_Details.QTY * ( CASE WHEN viewItem_Price.VATable = 0 THEN viewItem_Price.UnitPrice " & _
                    " WHEN VATInclusive = 1 THEN viewItem_Price.UnitPrice/1.12 " & _
                    " ELSE viewItem_Price.UnitPrice END ) AS GrossAmount, " & _
                    " CASE WHEN viewItem_Price.VATable = 0 THEN 0 " & _
                    " WHEN VATInclusive = 1 THEN viewItem_Price.UnitPrice/1.12*.12 " & _
                    " ELSE viewItem_Price.UnitPrice * 0.12 END AS VAT, " & _
                    " tblDR_Details.NetAmount,  viewItem_Price.VATable, viewItem_Price.VatInclusive, tblDR_Details.Size, tblDR_Details.Color " & _
                    " FROM tblDR " & _
                    " INNER JOIN tblDR_Details    " & _
                    " ON tblDR.TransID = tblDR_Details.TransID    " & _
                    " INNER JOIN viewItem_Price ON " & _
                    " viewItem_Price.ItemCode = tblDR_Details.ItemCode  AND Category ='Selling' AND PriceStatus = 'Active'  " & _
                    " WHERE  tblDR.TransID = '" & DR_No & "' "
            SQL.ReadQuery(query)
            While SQL.SQLDR.Read
                'itemcode = SQL.SQLDR("ItemCode").ToString
                'dgvItemList.Rows.Add("")
                'dgvItemList.Rows(dgvItemList.Rows.Count - 2).Cells(chItemCode.Index).Value = itemcode
                ...
                'dgvItemList.Rows(dgvItemList.Rows.Count - 2).Cells(chVATAmount.Index).Value = CDec(SQL.SQLDR("VAT")).ToString("N2")
                ...
                'dgvItemList.Rows(dgvItemList.Rows.Count - 2).Cells(dgcVATInc.Index).Value = SQL.SQLDR("VatInclusive")
                'dgvItemList.Rows(dgvItemList.Rows.Count - 2).Cells(chSize.Index).Value = SQL.SQLDR("Size").ToString
                'dgvItemList.Rows(dgvItemList.Rows.Count - 2).Cells(chColor.Index).Value = SQL.SQLDR("Color").ToString

                itemcode = SQL.SQLDR("ItemCode").ToString()
                dgvItemList.Rows.Add(itemcode, SQL.SQLDR("Description").ToString, SQL.SQLDR("UOM").ToString, _
                                     SQL.SQLDR("QTY").ToString, CDec(SQL.SQLDR("UnitPrice")).ToString("N4"), _
                                     CDec(SQL.SQLDR("GrossAmount")).ToString("N2"), _
                                      CDec(0).ToString("N2"), _
                                     CDec(0).ToString("N2"), _
                                     CDec(SQL.SQLDR("VAT")).ToString("N2"), _
                                     CDec(SQL.SQLDR("NetAmount")).ToString("N2"), _
                                     SQL.SQLDR("VATable").ToString, _
                                     SQL.SQLDR("VatInclusive").ToString, _
                                     SQL.SQLDR("Size").ToString, _
                                     SQL.SQLDR("Color").ToString)
                ...
                ' Recompute(dgvItemList.Rows.Count - 2, 4)
            End While
    ...
    Public Sub LoadItem(ByVal ID As String, ByVal ItemCode As String)
        ...
        query = " SELECT  ItemCode, ItemName, UOM, 1 AS QTY, VATable, " & _
                ...
                "         END AS VAT, VatInclusive, Size, Color " & _
        ...
        If SQL.SQLDS.Tables(0).Rows.Count > 0 Then
                ...
                dgvItemList.Rows.Add(New String() {row(0).ToString, row(1).ToString,
                                              row(2).ToString,
                                               row(3).ToString,
                                               CDec(row(5)).ToString("N2"), CDec(row(5)).ToString("N2"),
                                             "",
                                              "0.00",
                                              CDec(row(6)).ToString("N2"),
                                                CDec(row(5) + row(6)).ToString("N2"),
                                            row(4),
                                             row(7),
                                             row(8).ToString,
                                              row(9).ToString})
                LoadUOM(row(0).ToString, dgvItemList.Rows.Count - 2)
                LoadColor(row(0).ToString, dgvItemList.Rows.Count - 2)
                LoadSize(row(0).ToString, dgvItemList.Rows.Count - 2)
                Next
        End If
        'While SQL.SQLDR.Read
        '    dgvItemList.Rows.Add(New String() {SQL.SQLDR("ItemCode0").ToString, _
        '                                         SQL.SQLDR("ItemName1").ToString, _
        '                                         SQL.SQLDR("UOM2").ToString, _
        '                                         SQL.SQLDR("QTY3").ToString, _
        '                                         Format(SQL.SQLDR("UnitPrice5"), "#,###,###,###.00").ToString, _
        '                                         Format(SQL.SQLDR("UnitPrice5"), "#,###,###,###.00").ToString, _
        '                                         "", "0.00", _
        '                                         Format(SQL.SQLDR("VAT6"), "#,###,###,###.00").ToString, _
        '                                         Format(SQL.SQLDR("UnitPrice5") + SQL.SQLDR("VAT6"), "#,###,###,###.00").ToString, _
        '                                         SQL.SQLDR("VAtable4"), _
        '                                        SQL.SQLDR("VatInclusive7"), SQL.SQLDR("Size8").ToString, SQL.SQLDR("Color9").ToString})
        '    LoadUOM(ItemCode, dgvItemList.Rows.Count - 3)
        '    LoadColor(ItemCode, dgvItemList.Rows.Count - 3)
        '    LoadSize(ItemCode, dgvItemList.Rows.Count - 3)
        'End While
        ComputeTotal()
    End Sub
    ...
    Private Sub LoadColor(ItemCode As String, ByVal SelectedIndex As Integer)
        Try
            ...
            dgvCB.Items.Clear()
            While SQL.SQLDR2.Read
                dgvCB.Items.Add(SQL.SQLDR2("Color").ToString)
            End While
            dgvCB.Items.Add("")
        Catch ex As Exception
            SaveError(ex.Message, ex.StackTrace, Me.Name.ToString, ModuleID)
    ...
    Private Sub LoadSize(ItemCode As String, ByVal SelectedIndex As Integer)
        Try
            ...
            dgvCB.Items.Clear()
            While SQL.SQLDR2.Read
                dgvCB.Items.Add(SQL.SQLDR2("Size").ToString)
            End While
            dgvCB.Items.Add("")
        Catch ex As Exception
            SaveError(ex.Message, ex.StackTrace, Me.Name.ToString, ModuleID)
    ...
    Private Sub SaveSI()
        Try
            ...
            insertSQL = " INSERT INTO " & _
                        " tblSI  (TransID, SI_No, BranchCode, BusinessCode, VCECode, DateSI, Terms, DateDue, Remarks, DebitAccnt, " & _
                        "         GrossAmount, Discount, VATAmount, NetAmount, Currency, Exchange_Rate, VATable, VATInc, SO_Ref, PL_Ref, DR_Ref,  WhoCreated, Status) " & _
                        " VALUES (@TransID, @SI_No, @BranchCode, @BusinessCode, @VCECode, @DateSI, @Terms,  @DateDue, @Remarks, @DebitAccnt, " & _
                        "         @GrossAmount, @Discount, @VATAmount, @NetAmount,  @Currency, @Exchange_Rate, @VATable, @VATInc,  @SO_Ref, @PL_Ref, @DR_Ref, @WhoCreated, @Status) "
            SQL.FlushParams()
            SQL.AddParam("@TransID", TransID)
            ...
            For Each row As DataGridViewRow In dgvItemList.Rows
                If Not row.Cells(chQTY.Index).Value Is Nothing AndAlso Not row.Cells(chItemCode.Index).Value Is Nothing Then
                    insertSQL = " INSERT INTO " & _
                                ...
                                "                DiscountRate, Discount, NetAmount, LineNum, WhoCreated, Size, Color) " & _
                                " VALUES(@TransId, @ItemCode, @Description, @UOM, @QTY, @UnitPrice, @GrossAmount, @VATable, @VATInc, @VATAmount, " & _
                                "        @DiscountRate, @Discount, @NetAmount, @LineNum, @WhoCreated, @Size, @Color) "
                    'insertSQL = " INSERT INTO " & _
                    ...
                    SQL.AddParam("@Size", row.Cells(chSize.Index).Value.ToString)
                    SQL.AddParam("@Color", row.Cells(chColor.Index).Value.ToString)
                    SQL.AddParam("@LineNum", line)
                    SQL.AddParam("@WhoCreated", UserID)
                    SQL.ExecNonQuery(insertSQL)
                    line += 1
            ...
            insertSQL = " INSERT INTO " & _
                        " tblSJ (TransID, SJ_No, VCECode, BranchCode, BusinessCode, DateSJ, TotalAmount,  Currency, Exchange_Rate, Remarks, TransAuto, WhoCreated, Terms, DueDate, SIRef, RefID, Reftype, Status) " & _
    ...
    Private Sub UpdateSI()
        Try
            ...
            insertSQL = " UPDATE tblSI " & _
                        ...
                        "        Terms = @Terms, DateDue = @DateDue, Remarks = @Remarks, DebitAccnt = @DebitAccnt, VATable = @VATable, VATInc = @VATInc, " & _
                        "        GrossAmount = @GrossAmount, Discount = @Discount, VATAmount = @VATAmount, NetAmount = @NetAmount,  Currency = @Currency, Exchange_Rate = @Exchange_Rate," & _
            ...
            updateSQL = " UPDATE tblSJ " & _
                       ...
                       "        Terms = @Terms, DueDate = @DueDate, SIRef = @SIRef, RefID = @RefID, Reftype = @Reftype" & _
                       " WHERE  SIRef =(SELECT SI_No FROM tblSI WHERE TransID = '" & TransID & "') "
            SQL.FlushParams()
            SQL.AddParam("@VCECode", txtVCECode.Text)
            SQL.AddParam("@BranchCode", BranchCode)
            SQL.AddParam("@BusinessCode", BusinessType)
            ...
            selectsql = " SELECT  TransID FROM tblSJ WHERE  RefID = '" & TransID & "' AND Reftype ='SI'"
    ...
    Private Sub GenerateEntry()
        ...
        If txtGross.Text <> txtNet.Text And txtVAT.Text <> "0.00" Then
            ...
            Select Case colindex
                ...
                Case chUnitPrice.Index
                    ...
                    ElseIf IsNumeric(dgvItemList.Item(chUnitPrice.Index, e.RowIndex).Value) AndAlso IsNumeric(dgvItemList.Item(chQTY.Index, e.RowIndex).Value) Then
                        Recompute(e.RowIndex, e.ColumnIndex)
                        ComputeTotal()
                        dgvItemList.Item(chUnitPrice.Index, e.RowIndex).Value = CDec(dgvItemList.Item(chUnitPrice.Index, e.RowIndex).Value).ToString("N4")
                    End If
            ...
            If MsgBox("Updating Record, Click Yes to confirm", MsgBoxStyle.Information + MsgBoxStyle.YesNo, "JADE Message Alert") = MsgBoxResult.Yes Then
                ...
                Else
                    If Not IfExist(txtTransNum.Text) Then
                        ...
                        LoadSI(TransID)
                    Else
                        MsgBox("SI" & " " & txtTransNum.Text & " already exist!", MsgBoxStyle.Exclamation)
                    End If
                End If
    ...
    Private Function IfExist(ByVal ID As String) As Boolean
        Dim query As String
        query = " SELECT * FROM tblSI WHERE SI_No ='" & ID & "'  "
        SQL.ReadQuery(query)
        If SQL.SQLDR.Read Then
            Return True
        Else
