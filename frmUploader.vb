...
Public Class frmUploader
    ...
    Private Sub BackgroundWorker1_DoWork(sender As System.Object, e As System.ComponentModel.DoWorkEventArgs) Handles bgwUpload.DoWork
        ...
        ' GET REPORT NAME
        If reportName = "CV Upload" Then
            Obj = CType(range.Cells(2, 22), Excel.Range)
            ...
            ' GET REPORT PERIOD
            Obj = CType(range.Cells(15, 21), Excel.Range)
            If Not Obj.value Is Nothing AndAlso Obj.value.ToString <> "" Then
                    ...
                    If reportName = "CV Upload" Then
                        If i = startIndex Then
                            ...
                            If j = 0 Then
                                ...
                                AddColumn("APPLICATION DATE")
                                AddColumn("REFTYPE")
                                AddColumn("TRANSNO")
                                AddColumn("REFTRANSID")
                                AddColumn("BOOK")
                                AddColumn("VCECODE")
                                AddColumn("VCENAME")
                                AddColumn("ACCOUNT CODE")
                                AddColumn("ACCOUNT TITLE")
                                AddColumn("DEBIT")
                                AddColumn("CREDIT")
                                AddColumn("PARTICULARS")
                                AddColumn("CHECKNO")
                                AddColumn("REFNO")
                                AddColumn("REMARKS")
                                AddColumn("OTHER REF")
                                AddColumn("STATUS")
                                AddColumn("COST CENTER")
                                AddColumn("SL CODE")
                            ...
                            Else
                                ...
                                If j = 0 Then
                                    ...
                                    If IsDate(Obj.value) Then
                                        AddValue(Obj.value, rowCount - 1, j)
                                    Else
                                        ObjChecker = CType(range.Cells(i, 2), Excel.Range)
                                        If Not ObjChecker.value Is Nothing AndAlso ObjChecker.value.ToString <> "" Then
                                            AddValue(currentDate, rowCount - 1, j)
                                        Else
                                            Exit For
                                        End If
                                    End If
                                ...
                                ElseIf j = 1 Then
                                    If Not Obj.value Is Nothing AndAlso Obj.value.ToString <> "" Then
                                        AddValue(Obj.value.ToString, rowCount - 1, j)
                                    Else
                                        AddValue(currentTrans, rowCount - 1, j)
                                    End If
                                ElseIf j = 2 Then
                                    ...
                                    If Not Obj.value Is Nothing AndAlso Obj.value.ToString <> "" AndAlso Obj.value.ToString <> "-DO-" Then
                                        AddValue(Obj.value.ToString, rowCount - 1, j)
                                    Else
                                        AddValue(currentVCE, rowCount - 1, j)
                                    End If
                                Else
                                    AddValue(Obj.value, rowCount - 1, j)
                                End If
        ...
        Select Case reportName
            ...
            Case "CHECK VOUCHER-  EXPENSE REPORT"
                ...
                For Each row As DataGridViewRow In dgvTK.Rows
                    ...
                    End If
                    prevTime = DateTime.Now
                    If VCENameexist(row.Cells(6).Value.ToString) Then
                        currentVCECode = GetVCECode(row.Cells(6).Value.ToString)
                    ElseIf row.Cells(6).Value.ToString = "CANCELLED" Then
                        currentVCECode = "CANCELLED"
                    Else
                        Msg(row.Cells(6).Value.ToString & " does not exist in vendor masterfile!", MsgBoxStyle.Exclamation)
                        Exit For
                    End If
                    Dim JETransID As Integer
                    Dim ID As Integer
                    If Not CVexist(row.Cells(3).Value) Then
                        Dim AccntCode As String = ""
                        Dim tempCode As String
                        If row.Cells(3).Value = "CVTRD-18-00871" Then
                            MsgBox("A")
                        End If
                        ...
                        InsertCV(ID, row.Cells(3).Value, row.Cells(1).Value, currentVCECode, 0)
