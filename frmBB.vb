...
Public Class frmBB
    ...
    Dim ModuleID As String = "BB"
    Dim ColumnPK As String = "BB_No"
    Dim DBTable As String = "tblBB"
    ...
    Private Sub frmJV_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            If interestgen = False Then
                If TransID <> "" Then
                    If Not AllowAccess("BB_VIEW") Then
                        msgRestricted()
    ...
    Private Sub SaveBB()
        ...
        Try
            ...
            insertSQL = " INSERT INTO " & _
                        " tblBB (TransID, BB_No, VCECode, BranchCode, BusinessCode, DateBB, TotalAmount, Currency, Exchange_Rate, Remarks, LN_Ref, TransAuto, WhoCreated, MemTransfer) " & _
                        " VALUES(@TransID, @BB_No, @VCECode, @BranchCode, @BusinessCode, @DateBB, @TotalAmount, @Currency, @Exchange_Rate, @Remarks, @LN_Ref, @TransAuto, @WhoCreated, @MemTransfer)"
            SQL1.FlushParams()
            ...
            SQL1.AddParam("@DateBB", dtpDocDate.Value.Date)
            ...
            If LOAN_ID > 0 Then
                ...
                updateSQL = " UPDATE tblLoan SET Status ='Released', DateRelease = '" & dtpDocDate.Value.Date & "', RefType = 'BB', RefTransID = '" & TransID & "' WHERE TransID = '" & LOAN_ID & "'"
            ...
            If strRefNo.Contains("LN:") Then
                ...
                For i As Integer = 0 To count
                    Dim updateSQL As String
                    updateSQL = " UPDATE tblLoan SET Status ='Released', DateRelease = '" & dtpDocDate.Value.Date & "', RefType = 'BB', RefTransID = '" & TransID & "' WHERE Loan_No = '" & strRefNo.Split("-")(i) & "'"
                    SQL1.ExecNonQuery(updateSQL)
                Next
            End If
        ...
        Finally
            RecordActivity(UserID, ModuleID, Me.Name.ToString, "INSERT", "BB_No", txtTransNum.Text, BusinessType, IIf(interestBranchCode <> "", interestBranchCode, BranchCode), "", activityStatus)
            SQL1.FlushParams()
        End Try
    End Sub
    ...
    Private Sub UpdateBB()
        ...
        Try
            ...
            activityStatus = True
            updateSQL = " UPDATE tblBB " & _
                        " SET    BB_No = @BB_No, BranchCode = @BranchCode, BusinessCode = @BusinessCode,  VCECode = @VCECode, DateBB = @DateBB, " & _
                        "        TotalAmount = @TotalAmount, Remarks = @Remarks,  LN_Ref = @LN_Ref, WhoModified = @WhoModified, DateModified = GETDATE(), MemTransfer = @MemTransfer, Currency = @Currency, Exchange_Rate = @Exchange_Rate " & _
                        " WHERE  TransID = @TransID "
            SQL.FlushParams()
            SQL.AddParam("@TransID", TransID)
            SQL.AddParam("@BB_No", BBNo)
            SQL.AddParam("@BranchCode", IIf(interestBranchCode <> "", interestBranchCode, BranchCode))
            SQL.AddParam("@BusinessCode", BusinessType)
            SQL.AddParam("@VCECode", txtVCECode.Text)
            SQL.AddParam("@DateBB", dtpDocDate.Value.Date)
            ...
            If strRefNo.Contains("LN:") Then
                ...
                For i As Integer = 0 To count
                    Dim updateSQL1 As String
                        updateSQL1 = " UPDATE tblLoan SET Status ='Released', DateRelease = '" & dtpDocDate.Value.Date & "', RefType = 'BB', RefTransID = '" & TransID & "' WHERE Loan_No = '" & strRefNo.Split("-")(i) & "'"
        ...
        Catch ex As Exception
            ...
            Throw New Exception(ex.Message, ex)
        Finally
            RecordActivity(UserID, ModuleID, Me.Name.ToString, "UPDATE", "BB_No", txtTransNum.Text, BusinessType, IIf(interestBranchCode <> "", interestBranchCode, BranchCode), "", activityStatus)
            SQL.FlushParams()
        End Try
    End Sub

    Private Sub LoadBB(ByVal ID As String)
        Dim query, Currency As String
        query = " SELECT  TransID, BB_No, tblBB.VCECode, VCEName, DateBB, TotalAmount, Remarks, ISNULL(LN_Ref,0) as LN_Ref, tblBB.Status, ISNULL(MemTransfer,0) AS MemTransfer, Currency, ISNULL(Exchange_Rate,0) AS Exchange_Rate   " & _
                " FROM    tblBB LEFT JOIN tblVCE_Master " & _
                " ON      tblBB.VCECode = tblVCE_Master.VCECode " & _
                " WHERE   TransID ='" & ID & "' "
        SQL.ReadQuery(query)
        If SQL.SQLDR.Read Then
            dtpDocDate.MinDate = "01-01-1900"
            TransID = SQL.SQLDR("TransID").ToString
            BBNo = SQL.SQLDR("BB_No").ToString
            LOAN_ID = SQL.SQLDR("LN_Ref").ToString
            txtTransNum.Text = BBNo
            txtVCECode.Text = SQL.SQLDR("VCECode").ToString
            txtVCEName.Text = SQL.SQLDR("VCEName").ToString
            disableEvent = True
            dtpDocDate.Value = SQL.SQLDR("DateBB")
            disableEvent = False
    ...
    Private Sub LoadAccountingEntry(ByVal TransID As Integer)
        Try
            ...
            query = " SELECT JE_No, VCECode, VCEName, AccntCode, AccntTitle, Particulars, RefNo," & _
                    ...
                    " WHERE  RefType ='BB' AND RefTransID ='" & TransID & "' "
            SQL.ReadQuery(query)
            dgvEntry.Rows.Clear()
    ...
    Private Sub tsbNew_Click(sender As System.Object, e As System.EventArgs) Handles tsbNew.Click
        'If Not AllowAccess("BB_ADD") Then
        '    msgRestricted()
        'Else
        ClearText()
        TransID = ""
    ...
    Private Sub tsbEdit_Click(sender As System.Object, e As System.EventArgs) Handles tsbEdit.Click
        If Not AllowAccess("BB_EDIT") Then
            msgRestricted()
        Else
            EnableControl(True)
    ...
    Private Sub tsbSave_Click(sender As System.Object, e As System.EventArgs) Handles tsbSave.Click
        Try
            ...
            ElseIf TransAuto = False AndAlso IfExist(txtTransNum.Text) And TransID = "" Then
                MsgBox("BB" & " " & txtTransNum.Text & " already exist!", MsgBoxStyle.Exclamation)
            ElseIf TransID = "" Then
                If MsgBox("Saving New Record, Click Yes to confirm", MsgBoxStyle.Information + MsgBoxStyle.YesNo, "GR8 Message Alert") = MsgBoxResult.Yes Then
                    ...
                    If TransAuto Then
                        BBNo = GenerateTransNum(TransAuto, ModuleID, ColumnPK, DBTable)
                    Else
                        BBNo = txtTransNum.Text
                    End If
                    txtTransNum.Text = BBNo
                    SaveBB()
    ...
    Private Function IfExist(ByVal ID As String) As Boolean
        Dim query As String
        query = " SELECT * FROM tblBB WHERE BB_No ='" & ID & "'  "
        SQL.ReadQuery(query)
        If SQL.SQLDR.Read Then
            Return True
        Else
    ...
    Private Sub tsbCancel_Click(sender As System.Object, e As System.EventArgs) Handles tsbCancel.Click
        'If Not AllowAccess("BB_DEL") Then
        '    msgRestricted()
        'Else
        If txtTransNum.Text <> "" Then
            If txtStatus.Text <> "Cancelled" AndAlso MsgBox("Are you sure you want to cancel this record?", MsgBoxStyle.Exclamation + MsgBoxStyle.YesNo, "GR8 Message Alert") = MsgBoxResult.Yes Then
                Try
                    activityStatus = True
                    Dim updateSQL As String
                    updateSQL = " UPDATE  tblBB SET Status ='Cancelled' WHERE TransID = @TransID "
                    SQL.FlushParams()
                    SQL.AddParam("@TransID", TransID)
                    SQL.ExecNonQuery(updateSQL)

                    updateSQL = " UPDATE  tblJE_Header SET Status ='Cancelled' WHERE RefTransID = @RefTransID  AND RefType ='BB' "
                    SQL.FlushParams()
                    SQL.AddParam("@RefTransID", TransID)
                    SQL.ExecNonQuery(updateSQL)
                    ...
                    insertSQL = " INSERT INTO tblJE_Details(JE_No, AccntCode, VCECode, Debit, Credit, Particulars, RefNo, LineNumber, OtherRef) " & _
                                ...
                                " WHERE JE_No IN (SELECT DISTINCT JE_No FROM tblJE_Header WHERE RefType ='BB' AND RefTransID ='" & TransID & "') "
                    SQL.ExecNonQuery(insertSQL)
                ...
                Catch ex As Exception
                    ...
                    SaveError(ex.Message, ex.StackTrace, Me.Name.ToString, ModuleID)
                Finally
                    RecordActivity(UserID, ModuleID, Me.Name.ToString, "CANCEL", "BB_No", BBNo, BusinessType, IIf(interestBranchCode <> "", interestBranchCode, BranchCode), "", activityStatus)
                    SQL.FlushParams()
                End Try
            ElseIf txtStatus.Text = "Cancelled" AndAlso MsgBox("Are you sure you want to un-cancel this record?", MsgBoxStyle.Exclamation + MsgBoxStyle.YesNo, "GR8 Message Alert") = MsgBoxResult.Yes Then
                Try
                    activityStatus = True
                    Dim updateSQL As String
                    updateSQL = " UPDATE  tblBB SET Status ='Active' WHERE TransID = @TransID "
                    SQL.FlushParams()
                    SQL.AddParam("@TransID", TransID)
                    SQL.ExecNonQuery(updateSQL)

                    updateSQL = " UPDATE  tblJE_Header SET Status ='Saved' WHERE RefTransID = @RefTransID  AND RefType ='BB' "
                    SQL.FlushParams()
                    SQL.AddParam("@RefTransID", TransID)
                    SQL.ExecNonQuery(updateSQL)
                    ...
                    insertSQL = " DELETE FROM tblJE_Details " & _
                                " WHERE JE_No IN (SELECT DISTINCT JE_No FROM tblJE_Header WHERE RefType ='BB' AND RefTransID ='" & TransID & "') AND LineNumber = 0 "
                    SQL.ExecNonQuery(insertSQL)
                    Msg("Record cancelled successfully", MsgBoxStyle.Information)


                    If LOAN_ID > 0 Then
                        updateSQL = " UPDATE tblLoan SET Status ='Released', DateRelease = '" & dtpDocDate.Value & "', RefType = 'BB', RefTransID = '" & TransID & "' WHERE TransID = '" & LOAN_ID & "'"
                        SQL.ExecNonQuery(updateSQL)
                    End If
    ...
    Private Sub tsbPrevious_Click(sender As System.Object, e As System.EventArgs) Handles tsbPrevious.Click
        If BBNo <> "" Then
            Dim query As String
            query = "   SELECT Top 1 TransID FROM tblBB  " & _
                    ...
                    " 	   ) " & _
                    "     )   " & _
                    "    AND BB_No < '" & BBNo & "' ORDER BY BB_No DESC  "
            SQL.ReadQuery(query)
            If SQL.SQLDR.Read Then
                TransID = SQL.SQLDR("TransID").ToString
                LoadBB(TransID)
    ...
    Private Sub tsbNext_Click(sender As System.Object, e As System.EventArgs) Handles tsbNext.Click
        If BBNo <> "" Then
            ...
            query = "   SELECT Top 1 TransID FROM tblBB  " & _
                    ...
                    " 	   ) " & _
                    "     )   " & _
                    "    AND BB_No > '" & BBNo & "' ORDER BY BB_No ASC  "
            SQL.ReadQuery(query)
            If SQL.SQLDR.Read Then
                TransID = SQL.SQLDR("TransID").ToString
                LoadBB(TransID)
    ...
    Private Sub dgvEntry_CurrentCellDirtyStateChanged(sender As Object, e As System.EventArgs) Handles dgvEntry.CurrentCellDirtyStateChanged
        If eColIndex = chBranchCode.Index And TypeOf (dgvEntry.CurrentRow.Cells(chBranchCode.Index)) Is DataGridViewComboBoxCell Then
            dgvEntry.EndEdit()
        End If
    End Sub
    ...
    Private Sub tsbDelete_Click(sender As System.Object, e As System.EventArgs) Handles tsbDelete.Click
        If MsgBox("Are you sure you want to delete this transaction?", MsgBoxStyle.YesNo + MsgBoxStyle.DefaultButton2, "Delete") = MsgBoxResult.Yes Then
            ...
            deleteSQL = "DELETE FROM tblJE_Header WHERE RefType = 'BB' AND RefTransID = '" & TransID & "'"
            SQL.ExecNonQuery(deleteSQL)
                ...
                ElseIf i > 1 Then
                    ...
                    If dgvEntry.Columns.Count > j + addlCol Then
                        ...
                        ElseIf j = chAccntTitle.Index Then
                            ' Check if has valid AccountTitle
    ...
    Private Sub tsbDownload_Click(sender As System.Object, e As System.EventArgs) Handles tsbDownload.Click
        Dim fileName As String = "BB Uploader.xlsx"
        Dim xlApp As Excel.Application
        Dim xlWorkBook As Excel.Workbook
        Dim xlWorkSheet As Excel.Worksheet
        xlApp = New Excel.Application
        ...
        If My.Computer.FileSystem.FileExists(App_Path + "\" & templateName & ".xlsx") Then
            ...
            Do
                fileName = "BB Uploader -" & ctr.ToString & ".xlsx"
