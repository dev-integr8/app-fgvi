Public Class frmSJ
    ...
    Dim AccntCode, TAX_IV, TAX_EWT, TAX_DOV As String
    ...
    Private Sub ClearText()
        ...
        dtpPayDate.Value = Date.Today.Date
        dtpReceiving.Checked = False
        txtPayPeriod.Text = ""
    ...
    Private Sub SaveSJ()
        ...
        Try
            ...
            insertSQL = " INSERT INTO " &
                        " tblSJ (TransID, SJ_No, VCECode, BranchCode, BusinessCode, DateSJ, TotalAmount, " &
                        "       Currency, Exchange_Rate,  Remarks, LN_Ref, TransAuto, WhoCreated, MemTransfer, " &
                        "       Terms, DueDate, BSRef, SIRef, RE_Ref, Payroll_Period, PayDate, ReceivingDate, Status) " &
                        " VALUES(@TransID, @SJ_No, @VCECode, @BranchCode, @BusinessCode, @DateSJ, @TotalAmount, " &
                        "       @Currency, @Exchange_Rate,  @Remarks, @LN_Ref, @TransAuto, @WhoCreated, @MemTransfer, " &
                        "       @Terms, @DueDate, @BSRef, @SIRef, @RE_Ref, @Payroll_Period, @PayDate, @ReceivingDate, @Status)"
            SQL1.FlushParams()
            ...
            SQL1.AddParam("@DateSJ", dtpDocDate.Value.Date)
            ...
            SQL1.AddParam("@RE_Ref", txtRERef.Text)
            SQL1.AddParam("@Payroll_Period", txtPayPeriod.Text)
            SQL1.AddParam("@PayDate", dtpPayDate.Value.Date)
            If ForApproval = True Then SQL1.AddParam("@Status", "Draft") Else SQL1.AddParam("@Status", "Active")
            If dtpReceiving.Checked = True Then
                SQL1.AddParam("@ReceivingDate", dtpReceiving.Value.Date)
            Else
                SQL1.AddParam("@ReceivingDate", DBNull.Value)
            End If
            ...
            If BSTransID > 0 Then
                ...
                updateSQL = " UPDATE tblBilling_Header  SET EntryStatus ='1' WHERE TransID = '" & BSTransID & "'"
            ...
            For Each item As DataGridViewRow In dgvEntry.Rows
                If item.Cells(chAccntCode.Index).Value <> Nothing Then
                    insertSQL = " INSERT INTO " & _
                                " tblJE_Details(JE_No, AccntCode, VCECode, Debit, Credit, Particulars, RefNo, LineNumber, CostCenter, CIP_Code, ProfitCenter, BranchCode, VATType) " & _
                                " VALUES(@JE_No, @AccntCode, @VCECode, @Debit, @Credit, @Particulars, @RefNo, @LineNumber, @CostCenter, @CIP_Code, @ProfitCenter, @BranchCode, @VATType)"
                    SQL1.FlushParams()
                    SQL1.AddParam("@JE_No", JETransiD)
    ...
    Private Sub UpdateSJ()
        ...
        Try
            ...
            updateSQL = " UPDATE tblSJ " &
                        " SET    SJ_No = @SJ_No, BranchCode = @BranchCode, BusinessCode = @BusinessCode,  VCECode = @VCECode, DateSJ = @DateSJ, " &
                        "        TotalAmount = @TotalAmount, Currency = @Currency, Exchange_Rate = @Exchange_Rate,  Remarks = @Remarks,  LN_Ref = @LN_Ref, WhoModified = @WhoModified, DateModified = GETDATE(), " &
                        "        MemTransfer = @MemTransfer, Terms = @Terms, DueDate = @DueDate, BSRef = @BSRef, SIRef = @SIRef, @RE_Ref = @RE_Ref, Payroll_Period = @Payroll_Period, PayDate = @PayDate, ReceivingDate = @ReceivingDate  " &
            ...
            SQL.AddParam("@RE_Ref", txtRERef.Text)
            SQL.AddParam("@Payroll_Period", txtPayPeriod.Text)
            SQL.AddParam("@PayDate", dtpPayDate.Value.Date)
            If dtpReceiving.Checked = True Then
                SQL.AddParam("@ReceivingDate", dtpReceiving.Value.Date)
            Else
                SQL.AddParam("@ReceivingDate", DBNull.Value)
            End If
    ...
    Private Sub LoadSJ(ByVal ID As String)
        ...
        query = " SELECT  TransID, SJ_No, tblSJ.VCECode, VCEName, DateSJ, TotalAmount, Remarks, ISNULL(LN_Ref,0) as LN_Ref, tblSJ.Status, ISNULL(MemTransfer,0) AS MemTransfer, tblSJ.Terms, DueDate, ISNULL(BSRef,'') as BSRef, ISNULL(SIRef,'') as SIRef, ISNULL(RE_Ref,'') as RE_Ref , " &
                " Currency, ISNULL(Exchange_Rate,0) AS Exchange_Rate, Payroll_Period, ISNULL(PayDate,'1900-01-01') AS PayDate,  ISNULL(Reftype,'') AS Reftype, ISNULL(RefID,0) AS RefID, ISNULL(ReceivingDate,'1900-01-01')  AS ReceivingDate " &
                " FROM    tblSJ LEFT JOIN viewVCE_Master " &
                " ON      tblSJ.VCECode = viewVCE_Master.VCECode " &
                " WHERE   TransID ='" & ID & "' "
        SQL.ReadQuery(query)
        If SQL.SQLDR.Read Then
            ...
            txtRERef.Text = SQL.SQLDR("RE_Ref").ToString
            MemTransfer = SQL.SQLDR("MemTransfer").ToString
            txtPayPeriod.Text = SQL.SQLDR("Payroll_Period").ToString
            dtpPayDate.Value = SQL.SQLDR("PayDate").ToString
            If SQL.SQLDR("ReceivingDate") = "01-01-1900" Then
                dtpReceiving.Checked = False
            Else
                dtpReceiving.Checked = True
                dtpReceiving.Value = SQL.SQLDR("ReceivingDate")
            End If
    ...
    Private Sub LoadAccountingEntry(ByVal TransID As Integer)
        Try
            ...
            query = " SELECT JE_No, VCECode, VCEName, AccntCode, AccntTitle, Particulars, RefNo," & _
                    ...
                    " FROM   View_GL  " & _
                " WHERE JE_No = (SELECT  JE_No FROM tblJE_Header WHERE RefType = 'SJ' AND RefTransID = " & TransID & " AND isUpload <> 1) "
            SQL.ReadQuery(query)
            dgvEntry.Rows.Clear()
    ...
    Private Sub tsbCancel_Click(sender As System.Object, e As System.EventArgs) Handles tsbCancel.Click
        If Not AllowAccess("SJ_DEL") Then
            msgRestricted()
        Else
            If txtTransNum.Text <> "" Then
                ...
                ElseIf txtStatus.Text = "Cancelled" AndAlso MsgBox("Are you sure you want to un-cancel this record?", MsgBoxStyle.Exclamation + MsgBoxStyle.YesNo, "JADE Message Alert") = MsgBoxResult.Yes Then
                    Try
                        ...
                        updateSQL = " UPDATE  tblJE_Header SET Status = @Status WHERE RefTransID = @RefTransID  AND RefType ='SJ' "
                        SQL.FlushParams()
                        SQL.AddParam("@RefTransID", TransID)
                        If ForApproval Then
                            SQL.AddParam("@Status", "Draft")
    ...
    Private Sub tsbPrevious_Click(sender As System.Object, e As System.EventArgs) Handles tsbPrevious.Click
        If SJNo <> "" Then
            Dim query As String
            query = "   SELECT Top 1 TransID FROM tblSJ  " &
                    "   INNER JOIN tblBranch  ON	          " &
                    "   tblSJ.BranchCode = tblBranch.BranchCode    " &
                    "     WHERE          " &
                    " 	( tblSJ.BranchCode IN  " &
    ...
    Private Sub dgvEntry_CellContentClick(sender As System.Object, e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvEntry.CellContentClick
        If e.ColumnIndex = chCompute.Index Then
            ...
            f.ShowDialog(VATamount)
            If IsNothing(dgvEntry.Item(chDebit.Index, e.RowIndex).Value) Then
                dgvEntry.Item(chCredit.Index, e.RowIndex).Value = CDec(dgvEntry.Item(chCredit.Index, e.RowIndex).Value - f.EWTAmount).ToString("N2")
                dgvEntry.Item(chVCECode.Index, e.RowIndex).Value = f.VCECode
                dgvEntry.Item(chVCEName.Index, e.RowIndex).Value = GetVCEName(f.VCECode)
            Else
                dgvEntry.Item(chDebit.Index, e.RowIndex).Value = CDec(dgvEntry.Item(chDebit.Index, e.RowIndex).Value - f.EWTAmount).ToString("N2")
                dgvEntry.Item(chVCECode.Index, e.RowIndex).Value = f.VCECode
                dgvEntry.Item(chVCEName.Index, e.RowIndex).Value = GetVCEName(f.VCECode)
            End If

            If f.EWTAmount <> 0 Then
                dgvEntry.Rows.Add(f.TAX_CWT)
            ...
            If f.VATAmount <> 0 Then
                dgvEntry.Rows.Add(f.AR_OutputVAT)
    ...
    Private Sub FromLoansToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs)
    ...
    Sub LoadLoan(ByVal Loan As String)
        Try
            ...
            Dim query As String
            query = " SELECT	tblLoan.LoanCode, SetupUnearned, CASE WHEN (SELECT Employer_Name FROM tblCompany WHERE Employer_Name LIKE '%lending%') IS NULL THEN LoanAccount ELSE '11702' END, IntIncomeAccount, UnearnedAccount, IntRecAccount, " &
                    " 		    LoanAmount, IntAmount, tblLoan.IntAmortMethod, Loan_No, VCECode, LoanPayable, tblLoan.BranchCode " &
                    " FROM	    tblLoan_Type INNER JOIN tblLoan " &
                    " ON		tblLoan_Type.LoanCode = tblLoan.LoanCode " &
                    " WHERE     TransID = '" & Loan & "' "
            SQL.GetQuery(query)
            If SQL.SQLDS.Tables(0).Rows.Count > 0 Then
                ...
                query = "    SELECT TransID, AccountCode, Amount, Description, VCECode, " &
                        "       CASE WHEN SetupCharges = 1  " &
                        " 	    THEN '' ELSE   RefID END AS RefID    " &
                        "    FROM tblLoan_Details " &
    ...
    Private Sub dtpDocDate_ValueChanged(sender As System.Object, e As System.EventArgs) Handles dtpDocDate.ValueChanged
        If disableEvent = False Then
            If dtpReceiving.Checked = False Then
                ...
                If SQL.SQLDR.Read Then
                    ...
                    If DateMode = "Year" Then
                        dtpDueDate.Value = DateAdd(DateInterval.Year, CDec(Days), dtpReceiving.Value.Date) ' GET MATURITY DATE BASED ON TERMS AND STARTING DATE
                    ElseIf DateMode = "Month" Then
                        dtpDueDate.Value = DateAdd(DateInterval.Month, CDec(Days), dtpReceiving.Value.Date) ' GET MATURITY DATE BASED ON TERMS AND STARTING DATE
                    Else
                        dtpDueDate.Value = DateAdd(DateInterval.Day, CDec(Days), dtpReceiving.Value.Date) ' GET MATURITY DATE BASED ON TERMS AND STARTING DATE
                    End If
                End If
            End If
        End If
    ...
    Private Sub FromSavingsToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs)
    ...
    Private Sub FromMemberToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs) Handles FromMemberToolStripMenuItem.Click
        ...
        f.ShowDialog("PAYROLL BS")
        LoadBS(f.transID)
        f.Dispose()
    End Sub

    Sub LoadBS(ByVal BS_TransID As String)
        Dim query, Billing_No, VCECode As String
        Dim GrossAmount, VATamount, EWTamount, AmountDue As Decimal
        SetPayrollDatabase()
        query = " SELECT    TransID, Billing_No, tblBilling_Header.Payroll_Period, " &
                " 	        CASE WHEN tblBilling_Header.Payroll_Period = 'SIL' " &
                "               THEN BillingDate ELSE Paydate " &
                "           END AS Paydate, " &
                "           CASE WHEN tblBilling_Header.Payroll_Period = 'SIL' " &
                "                THEN Remarks + '(' + Org_Name + ')'  " &
                "                ELSE Remarks  " &
                "           END AS Remarks,  " &
                " 		GrossAmount, VATamount, (EWTamount * -1) AS EWTamount, AmountDue, BillingDate, tblBilling_Header.Status, VCECode  " &
                " FROM    tblBilling_Header LEFT JOIN tblPayroll_Period " &
                " ON		tblBilling_Header.Payroll_Period = tblPayroll_Period.Payroll_Period " &
                " LEFT JOIN tblEmployee_Group " &
                " ON		tblBilling_Header.OrgCost_ID = tblEmployee_Group.GroupID " &
                " WHERE   TransID = '" & BS_TransID & "'"
        SQL_RUBY.ReadQuery(query)
        dgvEntry.Rows.Clear()
        If SQL_RUBY.SQLDR.Read Then
            BSTransID = SQL_RUBY.SQLDR("TransID")
            txtPayPeriod.Text = SQL_RUBY.SQLDR("Payroll_Period")
            dtpPayDate.Value = SQL_RUBY.SQLDR("Paydate").ToString
            GrossAmount = CDec(SQL_RUBY.SQLDR("GrossAmount")).ToString("N2")
            VATamount = CDec(SQL_RUBY.SQLDR("VATamount")).ToString("N2")
            EWTamount = CDec(SQL_RUBY.SQLDR("EWTamount")).ToString("N2")
            AmountDue = CDec(SQL_RUBY.SQLDR("AmountDue")).ToString("N2")
            Billing_No = SQL_RUBY.SQLDR("Billing_No").ToString
            VCECode = SQL_RUBY.SQLDR("VCECode").ToString
            txtBSRef.Text = Billing_No
            SetDatabase()
            txtVCECode.Text = VCECode
            txtVCEName.Text = GetVCEName(VCECode)
            LoadVCETerms()
            'Amount Due
            If GrossAmount > 0 Then
                dgvEntry.Rows.Add("")
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chAccntCode.Index).Value = "1120210"
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chAccntTitle.Index).Value = GetAccntTitle("1120210")
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chDebit.Index).Value = CDec(AmountDue).ToString("N2")
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chCredit.Index).Value = ""
                ...
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chRefNo.Index).Value = "BS:" & Billing_No
            ...
            If EWTamount > 0 Then
                'EWT Entry
                dgvEntry.Rows.Add("")
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chAccntCode.Index).Value = TAX_EWT
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chAccntTitle.Index).Value = GetAccntTitle(TAX_EWT)
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chDebit.Index).Value = CDec(EWTamount).ToString("N2")
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chCredit.Index).Value = ""
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chVCECode.Index).Value = ""
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chVCEName.Index).Value = ""
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chParticulars.Index).Value = ""
            ...
            If VATamount > 0 Then
                'Input VAT Entry
                dgvEntry.Rows.Add("")
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chAccntCode.Index).Value = TAX_DOV
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chAccntTitle.Index).Value = GetAccntTitle(TAX_DOV)
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chDebit.Index).Value = ""
            ...
            If GrossAmount > 0 Then
                'GROSS AMOUNT
                dgvEntry.Rows.Add("")
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chAccntCode.Index).Value = "4010000"
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chAccntTitle.Index).Value = GetAccntTitle("4010000")
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chDebit.Index).Value = ""
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chCredit.Index).Value = CDec(GrossAmount).ToString("N2")
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chVCECode.Index).Value = ""
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chVCEName.Index).Value = ""
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chParticulars.Index).Value = ""
                dgvEntry.Rows(dgvEntry.Rows.Count - 2).Cells(chRefNo.Index).Value = "BS:" & Billing_No
    ...
    Private Sub dtpReceiving_ValueChanged(sender As Object, e As EventArgs) Handles dtpReceiving.ValueChanged
        If disableEvent = False Then
            If TransID = "" Then
                txtTransNum.Text = GenerateTransNum(TransAuto, ModuleID, ColumnPK, DBTable)
            End If
    ...
    Private Sub SJReceivingToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SJReceivingToolStripMenuItem.Click
        Dim f As New frmReport_Filter
        f.Report = "BS Receiving"
        f.ShowDialog("")
        f.Dispose()
    End Sub

    Private Sub BSNotYetReceivedToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles BSNotYetReceivedToolStripMenuItem.Click
        Dim f As New frmReport_Display
        f.ShowDialog("BS_ForReceiving", UserName)
        f.Dispose()
    End Sub

    Private Sub LoadSetup()
        Dim query As String
        query = " SELECT   TAX_IV, TAX_EWT,TAX_DOV " &
                " FROM tblSystemSetup "
        SQL.ReadQuery(query)
        If SQL.SQLDR.Read Then
    ...
    Private Sub dtpDueDate_ValueChanged(sender As System.Object, e As System.EventArgs) Handles dtpDueDate.ValueChanged
