Public Class frmAdvances
    ...
    Private Sub LoadADV(ID As String)
        Dim query As String
        query = " SELECT  tblADV.TransID, tblADV.ADV_No, tblADV.VCECOde, tblVCE_Master.VCEName, tblADV.DateADV, ISNULL(TotalAmount,0) AS TotalAmount, tblADV.AccntCode, " & _
                "         ISNULL(ADV_Percent,0) AS ADV_Percent, ISNULL(tblADV.ADV_Amount,0) AS ADV_Amount, tblADV.Remarks, tblADV.PO_Ref, View_ADV_Balance.Status  " & _
                " FROM    tblADV INNER JOIN tblVCE_Master " & _
                " ON      tblADV.VCECode = tblVCE_Master.VCECode " & _
                " INNER JOIN View_ADV_Balance " & _
                " ON      tblADV.TransID = View_ADV_Balance.TransID " & _
                " WHERE   tblADV.TransID ='" & ID & "' "
        SQL.ReadQuery(query)
    ...
    Private Sub LoadPO(ByVal ID As String)
        ...
        query = " SELECT    tblPO.TransID, tblPO.PO_No, DatePO AS Date, tblPO.VCECode, VCEName,  " & _
                ...
                " 		    Remarks, AccntCode, DP_Amount, (DP_Amount/NetAmount) * 100 AS AmountPercent " & _
                ...
                " ON        tblPO.TransID = tblPO_PartialPayment_Header.TransID " & _
        ...
        If SQL.SQLDR.Read Then
            ...
            txtAdvPercent.Text = CDec(SQL.SQLDR("AmountPercent")).ToString("N2")
            txtAdvAmount.Text = CDec(SQL.SQLDR("DP_Amount")).ToString("N2")
            txtRemarks.Text = SQL.SQLDR("Remarks").ToString
    ...
    Private Sub SaveADV()
        Try
            ...
            insertSQL = " INSERT INTO " & _
                    " tblADV    (TransID, ADV_No, BranchCode, BusinessCode, VCECode, DateADV,  TotalAmount, AccntCode, ADV_Percent,  ADV_Amount, Remarks, PO_Ref, WhoCreated, Status) " & _
                    " VALUES (@TransID, @ADV_No, @BranchCode, @BusinessCode, @VCECode, @DateADV, @TotalAmount, @AccntCode, @ADV_Percent, @ADV_Amount, @Remarks, @PO_Ref, @WhoCreated, @Status) "
            SQL.FlushParams()
