Public Class frmGI_Type
    Dim moduleID As String = "GI"
    Dim RecordID As String = ""
    Dim modTbl As String = "tblGI_Type"
    Dim modCol As String = "RecordID"

    Private Sub frmGI_Type_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            LoadPurchaseReturn()
            LoadInventoryCount()
            LoadConsignment()
        ...
        Try
            ...
            Else
                ...
                insertSQL = " INSERT INTO " & _
                            " tblGI_Type(RecordID, Description, DefaultAccount, Amount, WhoCreated) " & _
                            " VALUES(@RecordID, @Description, @DefaultAccount, @Amount,@WhoCreated)"
                SQL.FlushParams()
                SQL.AddParam("@RecordID", RecordID)
                SQL.AddParam("@Description", "Purchase Return")
    ...
    Private Sub LoadConsignment()
        Try
            Dim insertSQL, query As String
            activityStatus = True
            query = " SELECT   RecordID, Description, DefaultAccount, Amount " & _
                    " FROM     tblGI_Type  " & _
                    " WHERE    Status ='Active' AND Description ='Consigment Item'"
            SQL.FlushParams()
            SQL.ReadQuery(query)
            If SQL.SQLDR.Read Then
            ...
            Else
                ...
                SQL.FlushParams()
                SQL.AddParam("@RecordID", RecordID)
                SQL.AddParam("@Description", "Consigment Item")
                SQL.AddParam("@Amount", CDec(IIf(txtAmount.Text <> "", txtAmount.Text, "0.00")))
                SQL.AddParam("@DefaultAccount", txtAccntCode.Text)
                SQL.AddParam("@WhoCreated", UserID)
                SQL.ExecNonQuery(insertSQL)
    ...
    Private Sub tsbNew_Click(sender As System.Object, e As System.EventArgs) Handles tsbNew.Click
        If Not AllowAccess("GI_MNT") Then
            msgRestricted()
        Else
            txtAccntCode.Clear()
            txtAccntTitle.Clear()
    ...
    Private Function RecordExist(ByVal Record As String) As Boolean
        Dim query As String
        query = " SELECT * FROM tblGI_Type WHERE Description =@Description AND Status ='Active' "
        SQL.FlushParams()
    ...
    Private Sub tsbDelete_Click(sender As System.Object, e As System.EventArgs) Handles tsbDelete.Click
        ...
        Else
            If RecordID <> "" Then
                If txtDesc.Text <> "Material Transfer" Then
                    If MsgBox("Are you sure you want to delete this record?", MsgBoxStyle.Exclamation + MsgBoxStyle.YesNo, "GR8 Message Alert") = MsgBoxResult.Yes Then
                        Try
                            ...
                            deleteSQL = " UPDATE tblGI_Type SET Status = 'Inactive' WHERE RecordID = @RecordID "
                            SQL.FlushParams()
                            SQL.AddParam("@RecordID", RecordID)
