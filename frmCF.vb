...
#End Region
        ...
        For Each row As DataGridViewRow In dgvList.Rows
            If cbPurchType.SelectedItem = "Goods" Then
                ...
                ElseIf row.Cells(chItemCode.Index).Value <> Nothing AndAlso row.Cells(chQTY.Index).Value = Nothing Then
                    ...
                    valid = False
                    Exit For
                ElseIf row.Cells(chApprovedSupplier.Index).Value = Nothing AndAlso row.Cells(chItemDesc.Index).Value <> Nothing Then
                    Msg("Please select  approved supplier for this item!", MsgBoxStyle.Exclamation)
                    valid = False
                    Exit For
                ElseIf row.Cells(chItemCode.Index).Value <> Nothing AndAlso row.Cells(chQTY.Index).Value <> Nothing Then
                    ctr += 1
    ...
    Private Sub SaveCF()
        Try
            ...
            insertSQL = " INSERT INTO " & _
                         " tblCF(TransID, CF_No, BranchCode, BusinessCode, DateCF, PR_Ref, CostCenter, " & _
                         " GrossAmount, VATAmount, TotalAmount, PurchaseType, Remarks, DateNeeded, RequestedBy, WhoCreated, Status) " & _
                         " VALUES(@TransID, @CF_No, @BranchCode, @BusinessCode, @DateCF, @PR_Ref, @CostCenter, " & _
                         " @GrossAmount, @VATAmount, @TotalAmount, @PurchaseType, @Remarks, @DateNeeded, @RequestedBy, @WhoCreated, @Status)"
            SQL.FlushParams()
            ...
            updateSQL = "UPDATE tblPR SET Status ='Closed' WHERE TransID IN (" & PR_ID & ") "
            ...
            If txtTransNum.Text <> "" Then
                If MsgBox("Are you sure you want to cancel this record?", MsgBoxStyle.Exclamation + MsgBoxStyle.YesNo, "JADE Message Alert") = MsgBoxResult.Yes Then
                    Try
                        ...
                        deleteSQL = " UPDATE  tblCF SET Status ='Cancelled' WHERE TransiD = @TransID "
                        SQL.FlushParams()
                        SQL.AddParam("@TransID", TransID)
                        SQL.ExecNonQuery(deleteSQL)
    ...
    Private Sub LoadPR(ByVal TransNum As String)
        ...
        '    cbDeliverTo.Text = SQL.SQLDR("RequestedBy").ToString
        PR_ID = TransNum
        query = " SELECT   ItemGroup, ItemCode, Description, UOM, SUM(QTY) AS QTY   " & _
                " FROM      tblPR_Details " & _
                " WHERE     tblPR_Details.TransID IN (" & TransNum & ")  AND QTY > 0  " & _
                " GROUP BY  ItemGroup, ItemCode, Description, UOM "
        dgvList.Rows.Clear()
        SQL.GetQuery(query)
        Dim ctr As Integer = 0
        ...
        For Each row As DataRow In dt.Rows
            ...
            If QTY > MOQ Then ' IF PR QUANTITY IS GREATER THAN MOQ OF ITEM
            ...
            If dgvList.Item(chSupplier1.Index, ctr).Value <> "" Then
                LoadSupplierList(ctr, chSupplier1.Index)
            End If

            LoadRemoveSupplier(itemGroup, itemCode, chSupplier2.Index, ctr, dgvList.Item(chSupplier1.Index, ctr).Value, dgvList.Item(chSupplier3.Index, ctr).Value, dgvList.Item(chSupplier4.Index, ctr).Value)
            LoadRemoveSupplier(itemGroup, itemCode, chSupplier3.Index, ctr, dgvList.Item(chSupplier1.Index, ctr).Value, dgvList.Item(chSupplier3.Index, ctr).Value, dgvList.Item(chSupplier4.Index, ctr).Value)
            LoadRemoveSupplier(itemGroup, itemCode, chSupplier4.Index, ctr, dgvList.Item(chSupplier1.Index, ctr).Value, dgvList.Item(chSupplier3.Index, ctr).Value, dgvList.Item(chSupplier4.Index, ctr).Value)
            ctr += 1
        Next
        ComputeTotal()
        ...
        Try
            ...
            Select Case colindex
                ...
                Case chSupplier1.Index
                    LoadRemoveSupplier(dgvList.Item(dgcBOMgroup.Index, e.RowIndex).Value, dgvList.Item(chItemCode.Index, e.RowIndex).Value, chSupplier2.Index, e.RowIndex, dgvList.Item(chSupplier1.Index, e.RowIndex).Value, dgvList.Item(chSupplier3.Index, e.RowIndex).Value, dgvList.Item(chSupplier4.Index, e.RowIndex).Value)
                    LoadSupplierList(e.RowIndex, e.ColumnIndex)
                Case chSupplier2.Index
                    LoadRemoveSupplier(dgvList.Item(dgcBOMgroup.Index, e.RowIndex).Value, dgvList.Item(chItemCode.Index, e.RowIndex).Value, chSupplier3.Index, e.RowIndex, dgvList.Item(chSupplier1.Index, e.RowIndex).Value, dgvList.Item(chSupplier2.Index, e.RowIndex).Value, dgvList.Item(chSupplier4.Index, e.RowIndex).Value)
                    LoadSupplierList(e.RowIndex, e.ColumnIndex)
                Case chSupplier3.Index
                    LoadRemoveSupplier(dgvList.Item(dgcBOMgroup.Index, e.RowIndex).Value, dgvList.Item(chItemCode.Index, e.RowIndex).Value, chSupplier4.Index, e.RowIndex, dgvList.Item(chSupplier1.Index, e.RowIndex).Value, dgvList.Item(chSupplier2.Index, e.RowIndex).Value, dgvList.Item(chSupplier3.Index, e.RowIndex).Value)
                    LoadSupplierList(e.RowIndex, e.ColumnIndex)
    ...
    Private Sub LoadLastSupplier(Optional ByVal ItemGroup As String = "", Optional ByVal ItemCode As String = "", _
        ...
        Try
            ...
            query = " SELECT     DISTINCT VCEName " & _
                    ...
                    " ON         viewVCE_Master.VCECode = tblPO.VCECode " & _
                    " WHERE      tblItem_Master.ItemCode = '" & ItemCode & "' OR tblPO_Details.ItemGroup = '" & ItemGroup & "' " & _
                    "  UNION ALL " & _
                    "  SELECT VCEName FROM tblItem_Master " & _
                    "  INNER JOIN viewVCE_Master ON " & _
                    "  viewVCE_Master.VCECode = tblItem_Master.PD_Supplier " & _
                    " WHERE      tblItem_Master.ItemCode = '" & ItemCode & "' OR tblItem_Master.ItemGroup = '" & ItemGroup & "' " & _
                    " ORDER BY   VCEName  "
            SQL.ReadQuery(query)
            dgvCB.Items.Clear()
            ...
            query = "  SELECT PD_Supplier, VCEName, ISNULL(PD_UnitCost,0) AS PD_UnitCost FROM tblItem_Master " & _
                      "  INNER JOIN viewVCE_Master ON " & _
                      "  viewVCE_Master.VCECode = tblItem_Master.PD_Supplier " & _
                      " WHERE      tblItem_Master.ItemCode = '" & ItemCode & "'"
            SQL.ReadQuery(query)
            If SQL.SQLDR.Read Then
                dgvList.Rows(SelectedRow).Cells(SelectedColumn - 1).Value = SQL.SQLDR("PD_Supplier").ToString
                dgvList.Rows(SelectedRow).Cells(SelectedColumn + 1).Value = CDec(SQL.SQLDR("PD_UnitCost")).ToString("N2")
                temp = SQL.SQLDR("VCEName").ToString
            End If
            dgvCB.Value = temp
        Catch ex As Exception
            SaveError(ex.Message, ex.StackTrace, Me.Name.ToString, ModuleID)
        ...
        Try
            If TypeOf (dgvList.Rows(SelectedRow).Cells(SelectedColumn)) Is DataGridViewComboBoxCell Then
                ...
                query = " SELECT DISTINCT VCEName " & _
                        ...
                        "       WHERE      tblItem_Master.ItemCode = '" & ItemCode & "' OR tblPO_Details.ItemGroup = '" & ItemGroup & "' " & _
                        "    UNION ALL " & _
                        "       SELECT VCEName FROM viewVCE_Master WHERE VCECode IN (SELECT S1Code FROM tblCF_Details WHERE ItemCode = '" & ItemCode & "' OR ItemGroup = '" & ItemGroup & "') " & _
                        "    UNION ALL " & _
                        "       SELECT VCEName FROM viewVCE_Master WHERE VCECode IN (SELECT S2Code FROM tblCF_Details WHERE ItemCode = '" & ItemCode & "' OR ItemGroup = '" & ItemGroup & "')  " & _
                        "    UNION ALL " & _
                        "       SELECT VCEName FROM viewVCE_Master WHERE VCECode IN (SELECT S3Code FROM tblCF_Details WHERE ItemCode = '" & ItemCode & "' OR ItemGroup = '" & ItemGroup & "')  " & _
                        "    UNION ALL " & _
                        "       SELECT VCEName FROM viewVCE_Master WHERE VCECode IN (SELECT S4Code FROM tblCF_Details WHERE ItemCode = '" & ItemCode & "' OR ItemGroup = '" & ItemGroup & "')  " & _
                        "    UNION ALL " & _
                        "       SELECT VCEName FROM viewVCE_Master WHERE VCECode IN (SELECT S4Code FROM tblCF_Details WHERE ItemCode = '" & ItemCode & "' OR ItemGroup = '" & ItemGroup & "')  " & _
                        "  UNION ALL " & _
                        "  SELECT VCEName FROM tblItem_Master " & _
    ...
    Private Sub LoadRemoveSupplier(Optional ByVal ItemGroup As String = "", Optional ByVal ItemCode As String = "", _
                                 Optional ByVal SelectedColumn As Integer = -1, Optional ByVal SelectedRow As Integer = -1, _
                                 Optional ByVal Sup1 As String = "", Optional ByVal Sup2 As String = "", Optional ByVal Sup3 As String = "")
        Try
            If TypeOf (dgvList.Rows(SelectedRow).Cells(SelectedColumn)) Is DataGridViewComboBoxCell Then
                Dim dgvCB As New DataGridViewComboBoxCell
                dgvCB = dgvList.Item(SelectedColumn, SelectedRow)
                ...
                query = " SELECT DISTINCT VCEName " & _
                        ...
                        "  viewVCE_Master.VCECode = tblItem_Master.PD_Supplier " & _
                        " WHERE     ( tblItem_Master.ItemCode = '" & ItemCode & "' OR tblItem_Master.ItemGroup = '" & ItemGroup & "') " & _
                        " ) AS List  WHERE  VCENAme NOT IN ('" & Sup1 & "', '" & Sup2 & "', '" & Sup3 & "') "
                SQL.ReadQuery(query)
                dgvCB.Items.Clear()
                If SQL.SQLDR.HasRows Then
                    While SQL.SQLDR.Read
    ...
    Private Sub dgvList_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles dgvList.KeyDown
        If e.KeyCode = Keys.Enter Then
            ...
            End If
            e.Handled = True
        ElseIf (e.KeyCode = Keys.F10 AndAlso e.Shift) OrElse e.KeyCode = Keys.Apps Then
            e.SuppressKeyPress = True
    ...
    Private Sub ViewItemInformationToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ViewItemInformationToolStripMenuItem.Click
        ...
        If dgvList.CurrentCell IsNot Nothing Then
            If dgvList.CurrentCell.RowIndex <> -1 Then
                Dim code As String = dgvList.Rows(dgvList.CurrentCell.RowIndex).Cells(chItemCode.Index).Value.ToString
                If code <> "" Then
                    f.ShowDialog(code)
                Else
